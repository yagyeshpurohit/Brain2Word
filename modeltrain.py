...
from keras.losses import cosine_similarity, categorical_crossentropy
...
def mean_distance_loss(y_true, y_pred):
    ...
    for i in range((val+1)):
        if i == 0:
            total += (val*cosine_similarity(y_true,y_pred))
        else:
            rolled = tf.manip.roll(y_pred, i, axis=0)
            total_two -= cosine_similarity(y_true,rolled)
...
model.compile(loss= {'pred_class':categorical_crossentropy,'pred_glove':mean_distance_loss,'fMRI_rec':cosine_similarity},loss_weights=[1.0,glove_weight,class_weight],optimizer=optimizer, metrics=['accuracy'])  
#model_pre.compile(loss= {'pred_class':categorical_crossentropy,'pred_glove':mean_distance_loss,'fMRI_rec':cosine_similarity},loss_weights=[1.0,glove_weight,0.0],optimizer=optimizer, metrics=['accuracy']) 
model_mean.compile(loss= {'pred_class':categorical_crossentropy,'pred_glove':mean_distance_loss,'fMRI_rec':cosine_similarity, 'concat':mean_distance_loss, 'dense_mid':mean_distance_loss},loss_weights=[1.0,glove_weight,class_weight,2.2,2.0],optimizer=optimizer, metrics=['accuracy']) 
...
if class_model:
    ...
    print("class model = 1 in callback")
    ...
    print("class model = 0 in callback")
    callback_list.append(model_checkpoint_callback)
...
print(f'[data_fine,glove_fine,class_fine] : {len([data_fine,glove_fine,class_fine])}')
